...
added_note = "Simulated Annealing basic implementation"
...
# annealing process in metallurgy. A metal is heated to a high temperature
# and then slowly cooled, allowing atoms to settle into a low-energy
# crystalline structure. Analogously, SA explores the solution space by:
#   - Starting at a high "temperature" where worse solutions are frequently
#     accepted, enabling broad exploration of the search space.
#   - Gradually reducing the temperature so that worse solutions are
#     accepted less often, focusing the search on exploitation (refining
#     the current good solution).
#
# KEY IDEA: At temperature T, a worse solution with cost increase delta
# is accepted with probability exp(-delta / T). When T is large, this
# probability is close to 1 (accept almost anything). When T is small,
# only very slight worsening is tolerated.
#
# This basic implementation uses:
#   - A random initial tour (simple shuffle of city indices)
#   - 2-opt swaps as the sole neighbourhood operator
#   - Geometric (exponential) cooling schedule: T_new = T * alpha
#   - No reheating or adaptive mechanisms
############################################################################

# ========================== SA PARAMETERS ==========================
# These control the behaviour of the simulated annealing search.

# initial_temp: The starting temperature. A high value means the algorithm
# will initially accept most worse solutions, allowing it to explore widely
# and avoid getting trapped in the first local optimum it finds.
initial_temp = 10000

# final_temp: The stopping temperature. Once the temperature drops to this
# level, the algorithm is essentially performing greedy hill-climbing
# (only accepting improvements) and further cooling provides no benefit.
final_temp = 1

# cooling_rate (alpha): The geometric cooling factor applied each iteration.
# T_new = T_old * cooling_rate. A value close to 1 (e.g., 0.9999) gives
# very slow cooling, meaning more iterations at each temperature level and
# thus a more thorough search. A lower value (e.g., 0.99) cools faster
# but may miss good solutions.
# With alpha=0.9999, it takes ~92,000 iterations to cool from 10000 to 1:
#   10000 * 0.9999^n = 1  =>  n = ln(10000) / ln(1/0.9999) ≈ 92,100
cooling_rate = 0.9999

# max_iterations: A safety cap on the number of iterations. In practice,
# the 55-second time limit (checked inside the loop) is likely to be the
# binding constraint for larger city files.
...
def calculate_tour_length(a_tour):
    ...
    A tour is a permutation of city indices [c0, c1, c2, ..., c_{n-1}].
    The total length is the sum of edges:
        dist(c0, c1) + dist(c1, c2) + ... + dist(c_{n-2}, c_{n-1}) + dist(c_{n-1}, c0)
    The last term closes the cycle back to the starting city.

    Parameters:
        a_tour (list[int]): A permutation of city indices 0..num_cities-1.

    Returns:
        int: The total tour distance (sum of all edge weights in the cycle).
    """
    total_length = 0
    n = len(a_tour)
    ...
    # Close the cycle: add the edge from the last city back to the first
...
def generate_initial_tour():
    """
    Generate a random initial tour by creating a list [0, 1, ..., num_cities-1]
    and shuffling it into a random permutation using Fisher-Yates shuffle
    (via random.shuffle).

    This is the simplest possible initialisation strategy. The resulting tour
    is typically very poor quality (far from optimal), but SA is designed to
    improve any starting solution through iterative neighbourhood search.

    Returns:
        list[int]: A random permutation of city indices.
    """
    # Create an ordered list of all city indices
    initial_tour = list(range(num_cities))
    # Randomly reorder the cities to form an initial tour
...
def get_neighbor_2opt(current_tour):
    """
    Generate a neighbouring solution using a 2-opt swap.

    2-opt is the standard neighbourhood operator for TSP. It works by:
    1. Selecting two random positions i and j in the tour (where i < j).
    2. Reversing the sub-sequence of cities between positions i and j.

    Geometrically, this removes two edges from the tour:
        edge(tour[i-1], tour[i])  and  edge(tour[j], tour[j+1])
    and replaces them with:
        edge(tour[i-1], tour[j])  and  edge(tour[i], tour[j+1])
    while reversing the direction of travel through the segment [i..j].

    Example:
        tour = [A, B, C, D, E, F], i=1, j=4
        Reversed segment: [B, C, D, E] -> [E, D, C, B]
        Result: [A, E, D, C, B, F]

    The 2-opt move is attractive because:
    - It always produces a valid tour (a permutation).
    - It can "untangle" crossing edges, which are always suboptimal.
    - It has O(n) time complexity (for the reversal).

    Parameters:
        current_tour (list[int]): The current tour to generate a neighbour from.

    Returns:
        list[int]: A new tour with one 2-opt swap applied.
    """
    n = len(current_tour)
    # Make a copy so we don't modify the original tour
    new_tour = current_tour[:]
    # Pick two random indices i < j for the segment to reverse
    i = random.randint(0, n - 2)
    j = random.randint(i + 1, n - 1)
    # Reverse the segment between positions i and j (inclusive)
    # This is the core 2-opt operation
...
def simulated_annealing():
    """
    Main simulated annealing loop for the TSP.

    The algorithm follows the standard SA pseudocode from lectures:

        1. S <- generate_initial_solution()
        2. best <- S
        3. T <- initial_temp
        4. WHILE T > final_temp AND within time limit:
            4a. S' <- generate_neighbour(S)          [2-opt swap]
            4b. delta <- cost(S') - cost(S)
            4c. IF delta < 0 THEN                     [S' is better]
                    S <- S'
            4d. ELSE IF random() < exp(-delta/T) THEN [accept worse with probability]
                    S <- S'
            4e. IF cost(S) < cost(best) THEN
                    best <- S
            4f. T <- T * cooling_rate                 [geometric cooling]
        5. RETURN best

    The acceptance probability exp(-delta/T) is the Boltzmann distribution
    from statistical mechanics. Key properties:
    - When T is high: exp(-delta/T) ≈ 1, so almost all moves accepted.
    - When T is low:  exp(-delta/T) ≈ 0, so only improvements accepted.
    - Larger delta (bigger worsening) gives lower acceptance probability.
    This provides a smooth transition from exploration to exploitation.

    Returns:
        tuple: (best_tour, best_length) - the shortest tour found and its length.
    """
    # ---- STEP 1: Generate the initial solution ----
    # Start with a completely random tour. This is typically a very poor
    # solution, but SA will iteratively improve it.
    ...
    # SA can accept worse solutions (to escape local optima), so the
    # current solution may not always be the best we've seen. We keep
    # a separate record of the overall best.
    best_tour = current_tour[:]
    best_length = current_length

    # ---- STEP 2: Initialise the temperature ----
    temp = initial_temp

    # ---- STEP 3: Main SA loop ----
    # Continue until: (a) temperature drops below threshold, OR
    #                 (b) max iterations reached, OR
    #                 (c) 55-second time limit approached
    iteration = 0
    while temp > final_temp and iteration < max_iterations and (time.time() - start_time) < 55:

        # STEP 3a: Generate a neighbouring solution via 2-opt swap
        # This creates a small perturbation of the current tour
        neighbor_tour = get_neighbor_2opt(current_tour)
        neighbor_length = calculate_tour_length(neighbor_tour)

        # STEP 3b: Calculate delta = cost(neighbour) - cost(current)
        # If delta < 0, the neighbour is shorter (better).
        # If delta > 0, the neighbour is longer (worse).
        delta = neighbor_length - current_length

        # STEP 3c & 3d: Metropolis acceptance criterion
        if delta < 0:
            # The neighbour is BETTER (shorter tour) — always accept it
            current_tour = neighbor_tour
            current_length = neighbor_length
        else:
            # The neighbour is WORSE (longer tour) — accept with probability
            # exp(-delta / T). This is the key mechanism that allows SA to
            # escape local optima by occasionally accepting uphill moves.
            # As T decreases, this probability shrinks, and the algorithm
        ...
        # STEP 3e: Update the global best if the current solution improves on it
        if current_length < best_length:
            best_tour = current_tour[:]
            best_length = current_length

        # STEP 3f: Geometric cooling — reduce temperature by factor alpha
        # This is the simplest and most common cooling schedule.
        # After k iterations: T_k = initial_temp * (cooling_rate)^k
        temp = temp * cooling_rate
        iteration += 1

    # STEP 4: Return the best tour found across all iterations
    return best_tour, best_length


# ========================== RUN THE ALGORITHM ==========================
# Execute SA and store the result in the reserved variables 'tour' and
# 'tour_length' that the template code expects.
